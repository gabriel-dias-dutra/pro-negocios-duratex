...
- `acoes[]` — Lista ordenada de ações, cada uma com:
  ...
  - `dificuldade` — (opcional) "FÁCIL", "MÉDIA" ou "DIFÍCIL" (Profundidade Geral e Volume usam esses rótulos para indicar o esforço envolvido).
...
6. CENÁRIO "SUBIR COM REDE":
   ...
   - **IMPORTANTE**: Renderize as ações EXATAMENTE na ordem em que aparecem em `cenarios.subirComRede.acoes[]`. No cenário com Rede, a Performance da Rede vem propositalmente como última ação no array (depende da equipe, não do cliente). Não reordene.
